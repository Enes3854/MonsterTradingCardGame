...
public class TradeManager {
    ...
    // Showcased possibilities to make trades with cards
    ...
    // Cancels a trade, removes the card from the marketplace
    ...
    // If conditions are true the trade finalizes
    public boolean tradeCards(User user, String tradeID, String cardID){
        ...
        try {
            ...
            float minDamage;
            String type;
            // Check if exchanging card is not in marketplace
            Connection conn = DataBaseService.getInstance().getConnection();
            if (marketplaceContains(cardID)){
                return false;
            }
    ...
    // See if there is a unique letter at the market and it can be traded, if conditions true: finalize the exchange.
